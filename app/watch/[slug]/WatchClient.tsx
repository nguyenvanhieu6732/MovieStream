...
type WatchClientProps = {
  slug: string;
  initialMovie: any;
  initialRelatedMovies: RelatedMovie[];
  initialPeoples: MoviePerson[];
  initialIsLongDescription: boolean;
};
...
export default function WatchClient({
  ...
  initialPeoples,
  initialIsLongDescription,
}: WatchClientProps) {
  const movie = initialMovie;
  const [isPlaying, setIsPlaying] = useState(false);
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [showAllEpisodes, setShowAllEpisodes] = useState(false);
  const [relatedMovies] = useState<RelatedMovie[]>(initialRelatedMovies);
  const [peoples] = useState<MoviePerson[]>(initialPeoples);
